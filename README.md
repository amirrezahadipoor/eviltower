...
├── ui/GameViewModel.kt           MVVM، حلقه‌ی ۳۰fps، شروع/پایان دور و ذخیره‌ی نتیجه
...
- `HP(w) = 24 × 1.045^(w-1)`؛ رشد مجاور ۴٫۵٪ و بدون جهش ناگهانی.
- `speed(type,w) = baseSpeed(type) × 1.004^(w-1)`.
- `count(w) = 7 + floor(w^0.72)`.
- پاداش کشت: `ceil(baseReward × (1 + 0.018 × (w-1)^0.82))`.
- پاداش پاک‌سازی: `ceil(20 × w^0.55)`؛ اقتصاد عمداً آهسته‌تر از قدرت دشمن رشد می‌کند.
...
- `BossHP = regularHP × (13 + w/45)` و `MiniBossHP = regularHP × (7.5 + w/90)`.
- از موج ۳۰ همه‌ی خانواده‌های دشمن وارد چرخش می‌شوند و هر ۳۵ موج نسخه‌ی بصری/نخبگی با ضریب
  `1 + 0.18 × floor((w-1)/35)` دیده می‌شود.
...
و `برج جادوی اهریمنی` که در فروشگاه باز می‌شود. فقط کماندار آسمان خفاش‌ها و دیگر دشمنان پرنده
...
- [ ] فروشگاه، بازکردن برج اهریمنی و پاداش شروع پس از restart برنامه باقی می‌ماند.
